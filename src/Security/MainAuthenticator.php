...
class MainAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    {
        $nni = $request->getPayload()->getString(key: 'nni');

        $request->getSession()->set(name: SecurityRequestAttributes::LAST_USERNAME, value: $nni);

        return new Passport(
            userBadge: new UserBadge(userIdentifier: $nni),
